...
public class Bookticket extends Activity {
	...
	TextView tvFair, tvBal;
	Button btnBookTicket, btnBack;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences myPrefs = this.getSharedPreferences("myPrefs",
				MODE_WORLD_READABLE);
		bal = myPrefs.getString("Bal", null);
		atvm = myPrefs.getString("AtvmNo", null);
		tvBal.setText("Your Balance is " + bal);
		...
		btnBookTicket = (Button) findViewById(R.id.btnBook);
		...
		btnBookTicket.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (!edAdult.getText().toString().trim().equals("")
					...
					if (Integer.parseInt(tvFair.getText().toString().trim()) > Integer
							.parseInt(bal)) {
						Toast.makeText(getApplicationContext(),
								"Insufficient Bal", Toast.LENGTH_LONG).show();
					} else {
						try {
							Log.d("ID",Id);
							int balamt = (new Integer(bal).intValue())
									- Integer.parseInt(tvFair.getText()
											.toString().trim());
							HttpClient httpclient = new DefaultHttpClient();
							HttpPost httppost = new HttpPost(
									"http://192.168.43.99/queeue/bookticket.php");
							List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(
									7);
							nameValuePairs.add(new BasicNameValuePair("source",
							...
							nameValuePairs.add(new BasicNameValuePair(
									"noChild", edchild.getText().toString()
											.trim()));
							nameValuePairs.add(new BasicNameValuePair("bal",
							...
							if (res.trim().contains("sucess")) {
								...
								SharedPreferences myPrefs = Bookticket.this
										.getSharedPreferences("myPrefs",
												MODE_WORLD_READABLE);
								SharedPreferences.Editor prefsEditor = myPrefs
										.edit();
								prefsEditor.putString("Bal",
										String.valueOf(balamt));
								prefsEditor.commit();
								bal=String.valueOf(balamt);
								tvBal.setText("Your Balance is " +balamt);
								Toast.makeText(
								...
								Toast.makeText(getApplicationContext(),
										...
										.show();
								Intent intent = new Intent(Bookticket.this,
										Home.class);
